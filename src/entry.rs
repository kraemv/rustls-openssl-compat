...
entry! {
    pub fn _PEM_read_bio_PrivateKey(
        ...
        let mut key_bytes = Bio::new_pair(Some(bp), None);
        let Ok(key) = crate::read_pem_key(&mut key_bytes, &[]) else { return ptr::null_mut()};
        let key = to_arc_mut_ptr(NotThreadSafe::new(key));
        if !x.is_null() {
            unsafe { ptr::write(x, key); }
        }
        mem::forget(key_bytes);
        key
    }
}
...
pub(crate) fn use_private_key_file(file_name: &str, file_type: c_int) -> Result<EvpPkey, Error> {
    ...
    EvpPkey::new_from_private_der_bytes(der_data)
}
